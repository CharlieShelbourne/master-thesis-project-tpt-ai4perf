...
class Encoder(nn.Module):
    ...
    def __init__(
            ...
            d_word_vec=27, d_model=512, d_inner_hid=1024, dropout=0.1):
...
class Decoder(nn.Module):
    ...
    def __init__(
            ...
            d_word_vec=50, d_model=512, d_inner_hid=1024, dropout=0.1):
...
class AttentionIsAllYouNeed(nn.Module):
    def __init__(self, n_layers=6, n_head=27,
                 d_word_vec=27, d_model=27, d_inner_hid=256, d_k=32, d_v=32,
                 dropout=0.1, proj_share_weight=True, embs_share_weight=True):
        ...
        self.encoder = Encoder(
             50, n_layers=n_layers, n_head=n_head,
            d_word_vec=d_word_vec, d_model=d_model,
            d_inner_hid=d_inner_hid, dropout=dropout)
        self.hidden2label = nn.Linear(50 * d_model, 3)
        self.batch_size = 200

    def forward(self, inp):
        src_seq = inp
        #         enc_output, *_ = self.encoder(src_seq, src_pos)   #64x200x512
        enc_output = self.encoder(src_seq)  # 64x200x512
