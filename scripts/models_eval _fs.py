...
if __name__ == '__main__':
    ...
    df_results = pd.DataFrame(
        columns=[[["fx","fxL","ps",'pcpPINJ1','pcpPINJ2','pcpDC1','pcpDC2']]],
        index=['instructions ROC', 'instructions acc',
            'branches ROC','branches acc', 'branch-misses ROC','branch-misses acc', 'branch-load-misses ROC',
            'branch-load-misses acc','cache-misses ROC',
            'cache-misses acc', 'cache-references ROC','cache-references acc',
            'cycles ROC', 'cycles acc','context-switches ROC','context-switches acc',
            'minor-faults ROC','minor-faults acc','page-faults ROC','page-faults acc',
            'L1-dcache-load-misses ROC','L1-dcache-load-misses acc','L1-dcache-loads ROC','L1-dcache-loads acc',
            'L1-dcache-stores ROC','L1-dcache-stores acc','L1-icache-load-misses ROC','L1-icache-load-misses acc',
            'LLC-load-misses ROC','LLC-load-misses acc','LLC-store-misses ROC','LLC-store-misses acc',
            'LLC-stores ROC','LLC-stores acc', 'LLC-loads ROC','LLC-loads acc',
            'dTLB-stores ROC','dTLB-stores acc', 'dTLB-load-misses ROC','dTLB-load-misses acc',
            'dTLB-store-misses ROC','dTLB-store-misses acc',
            'iTLB-loads ROC', 'iTLB-loads acc', 'iTLB-load-misses ROC','iTLB-load-misses acc',
            'node-load-misses ROC','node-load-misses acc',
            'node-loads ROC','node-loads acc', 'node-store-misses ROC','node-store-misses acc',
            'node-stores ROC','node-stores acc'])
...
########################################## TEST PRE-SAVED MODELS #######################################################
                ...
                net.load_state_dict(torch.load('/home/francesco/Documents/Thesis_project/Models_fs/' + model + '_' + tag_train + '_'+ leave_out+'.pt',
                ...
                df_conf_mtrx.to_csv('/home/francesco/Documents/Thesis_project/Results/ML/Feature_strength/Confusion_matrix/'+ model +'_' + tag_train + '_'+leave_out+'.csv')
                ...
                df_results.loc[leave_out + ' ROC',tag_test] = round(eval['ROC'].iloc[0], 3)#("ROC: "+str(round(eval['ROC'].iloc[0], 3)),"F1: " +str(round(eval['F1'].iloc[0], 3)))
                df_results.loc[leave_out + ' acc',tag_test] = round(eval['acc'].iloc[0], 3)
    df_results.to_csv('/home/francesco/Documents/Thesis_project/Results/ML/Feature_strength/'+ model +'/_evaluation_matrix.csv')